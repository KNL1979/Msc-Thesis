...
def run_dashboard(params):
    ...
    # Load tagpie_data from JSON file
    with open('data/tagpie_data.json', 'r') as file:
        tagpie_data = json.load(file)
    ...
    ### TAGPIE ###
    def generate_tagpie_html(tagpie_data):
        html_code = f"""<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>TagPie Example</title>
        <!-- Include necessary CSS -->
        <link rel="stylesheet" type="text/css" href="tagpie/TagPie.css">
        <!-- Include jQuery -->
        <script src="tagpie/jquery.min.js"></script>
    </head>
    <body>
    
    <!-- TagPie container -->
    <div id="tagpie-container"></div>
    
    <!-- Include necessary JavaScript -->
    <script src="tagpie/d3.v3.min.js"></script>
    <script src="tagpie/d3.layout.cloud.tagpies.js"></script>
    <script src="tagpie/TagPie.js"></script>
    
    <script>
        // Load tagpie_data from JSON file
        var tagpie_data = {tagpie_data};
    
        // Initialize TagPie
        new TagPie("tagpie-container", tagpie_data);
    </script>
    ...
    tagpie_container = html.Div(id='tagpie-container')
    interval_component = dcc.Interval(id='interval-component', interval=60*1000, n_intervals=0)
    
    # Generate the TagPie HTML code
    tagpie_html_content = generate_tagpie_html(json.dumps(tagpie_data))
    ...
    # Define a function to generate the JavaScript code for TagPie initialization
    def generate_tagpie_js(container_id, tagpie_data):
        js_code = f'new TagPie("{container_id}", {json.dumps(tagpie_data)});'
        return js_code
    
    # Update the callback function to return the JavaScript code
    @app.callback(
        Output('tagpie-container', 'children'),
        [Input('interval-component', 'n_intervals')]
    )
    def update_tagpie(n):
        # Return JavaScript code for initializing TagPie
        return html.Script(generate_tagpie_js('tagpie-container', tagpie_data))
    ...
    app.layout = dbc.Container(
        [
            ...
            # Row containing text window, map, and TagPie
            dbc.Row([
                ...
                # Column for the TagPie
                dbc.Col(
                    [
                        html.Div([
                        dcc.Markdown(tagpie_html_content)
                    ]),
                        interval_component
                    ],
                    width=4
            ...
            # Row containing reset button
            dbc.Row([
                # Column containing reset button
                dbc.Col(
                    reset_button,
                    width=12
                )
