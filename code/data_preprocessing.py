...
# Detect Dutch sentences using langdetect and create Dutch chunks
dutch_chunks = []
for chunk in tqdm(chunks, desc="Detecting Dutch Sentences"):
    try:
        language = detect(chunk)
        if language == 'nl' and not re.search(exclude_pattern, chunk, flags=re.IGNORECASE) and len(chunk.split()) >= 50:
            dutch_chunks.append(chunk)
    except Exception as e:
        # Handle any exceptions (e.g., short or ambiguous text)
        pass

# Save the Dutch chunks
